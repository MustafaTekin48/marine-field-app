...
export default function SuScreen({ route }) {
  ...
  const totalCost = parseFloat(ton) * 6 || 0;

  const handleSave = () => {
    if (!selectedBoat || !serviceDate || !ton) {
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <KeyboardAwareScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        <Text style={styles.title}>🚰 Su Kullanımı</Text>
        ...
        <TextInput
          ...
          placeholder="Kullanılan su miktarı (ton)"
          value={ton}
          onChangeText={setTon}
        />
